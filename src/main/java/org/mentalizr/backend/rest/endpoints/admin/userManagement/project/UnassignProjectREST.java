...
import org.mentalizr.serviceObjects.userManagement.ProjectUnassignSO;
...
@Path("v1")
public class UnassignProjectREST {

    private static final String SERVICE_ID = "/admin/user/project/unassign";

    @POST
    @Path(SERVICE_ID)
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response create(ProjectUnassignSO projectUnassignSO,
        ...
        return new Service(httpServletRequest){
            ...
            protected void checkPreconditions() throws ServicePreconditionFailedException, M7rInfrastructureException {
                AssertRolePatient.existsAsAssigned(projectUnassignSO.getUserId());
            }

            @Override
            protected Object workLoad() throws DataSourceException {
                try {
                    RolePatientVO rolePatientVO = RolePatientDAO.load(projectUnassignSO.getUserId());
                    rolePatientVO.setProjectId(null);
